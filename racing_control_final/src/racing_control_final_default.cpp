...
namespace {
int ToDirectionSign(float default_direction) {
    // 约定：>=0 表示 LEFT，<0 表示 RIGHT；即使为 0 也默认 LEFT
    return (default_direction >= 0.0f) ? 1 : -1;
}
}  // namespace
...
void RacingControlFinal::TargetCallback(const obstacle_detector::PerceptionTargets::ConstPtr& msg) {
    ...
    if (current_state_ == SessionState::SESSION_4) {
        ...
        if (zebra_found) {
            ...
            if (!collecting_sign_center_ && zebra_best_bottom >= zebra_direction_start_bottom) {
                collecting_sign_center_ = true;
                ROS_INFO("Session 4: Start collecting sign center (zebra y2=%.1f >= %.1f)",
...
void RacingControlFinal::Execute() {
    switch (current_state_) {
        ...
        case SessionState::SESSION_4:
            ...
            // 检查跳变条件：停车超过 wait_timeout_ 后，使用 default_direction_ 作为最终转向方向
            if (stop_started_) {
                double elapsed = (ros::Time::now() - stop_start_time_).toSec();
                if (elapsed >= wait_timeout_) {
                    const int final_dir = ToDirectionSign(default_direction_);
                    direction_.store(final_dir);

                    // 根据指示牌中心位置计算“第一次转向”的动态时间缩放（可选）
                    if (!enable_dynamic_turn_from_sign_) {
                        ...
                        if (!has_sign_center) {
                            ...
                            turn_angle_from_sign_ = (final_dir > 0) ? turn_angle_left_ : turn_angle_right_;
                            turn_duration_from_sign_ = (final_dir > 0) ? turn_duration_left_ : turn_duration_right_;
                            use_dynamic_turn_from_sign_ = false;
                            ...
                            ComputeTurnFromSignCenter(final_dir, cx, turn_angle_from_sign_, turn_duration_from_sign_, turn_scale_from_sign_);
                            use_dynamic_turn_from_sign_ = true;
                            ROS_WARN("Dynamic turn from sign: dir=%s, cx=%.1f => scale=%.3f, angle=%.2f, duration=%.2f",
                                     final_dir > 0 ? "LEFT" : "RIGHT", cx, turn_scale_from_sign_, turn_angle_from_sign_, turn_duration_from_sign_);
                        }
                    }

                    ROS_WARN("========== SESSION 4 → SESSION 5 ==========");
                    ROS_INFO("Waited %.1fs, using default direction=%s, switching to Session 5",
                             elapsed, final_dir > 0 ? "LEFT" : "RIGHT");
                    current_state_ = SessionState::SESSION_5;
                    
                    // 初始化 Session 5
                    original_direction_ = final_dir;
                    turn_count_ = 0;
                    y_obs_flag_ = 0;
...
void RacingControlFinal::ExecuteSession4() {
    ...
    if (!stop_started_) {
        ...
        stop_started_ = true;
        ROS_INFO("Session 4: Stopped at zebra crossing, waiting %.1fs then using default direction...", wait_timeout_);
    }
    ...
    double elapsed = (ros::Time::now() - stop_start_time_).toSec();
    ROS_INFO_THROTTLE(2, "Session 4: Waiting... (%.1fs / %.1fs)", elapsed, wait_timeout_);
}
